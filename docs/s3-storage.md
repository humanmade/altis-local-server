...
stored in S3 via the S3 Uploads plugin, and synced back to `content/uploads` on the host automatically.

## S3 Commands

Use `composer server s3 import-uploads` to sync existing files from `content/uploads` on the host into the S3 bucket. This is
...
Files placed directly in `content/uploads` on the host are not automatically synced to S3. Use `import-uploads` to sync them.

The sync service runs every 10 seconds. If uploads are not appearing in `content/uploads`, check the sync service logs:
