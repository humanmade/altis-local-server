...
To get details on the running Local Server status and containers, run `composer serve status`. You should see output similar to:
...
All containers should have a status of "Up". If they do not, you can inspect the logs for each service by running `composer serve logs <service>`, for example, if `site_db_1` shows a status other than "Up", run `composer serve logs db`.
